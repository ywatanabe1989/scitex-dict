...
# the shared ci-cpu.sif bakes scitex-dev[all,dev] DEPS, NOT scitex-dict's —
...
# so only scitex-dict's own thin dep set is fetched per run.
#
...
export TMPDIR="/tmp/ci-scitex_dict-${GITHUB_RUN_ID:-0}-${GITHUB_RUN_ATTEMPT:-0}-$V"
rm -rf "$TMPDIR"
...
# scitex-dict's reproducibility tests (validate_recipe renders the SAME recipe
...
# Install scitex-dict + its [all,dev] extras WITH deps into the writable target.
# Fallback chain mirrors scitex-dict's historical bare-uv/pip workflow so a
...
# scitex-dict's suite is ~2460 tests; single-process it overran the job's old
...
# scitex-dict style-stack are naturally isolated per worker — the safe way to
# parallelise a matplotlib-heavy suite.
#
# Worker count: use ALL cores. Each matrix leg now runs on its own dedicated
# self-hosted node (one runner per node: scitex-dict-01/02/03), so there is no
...
# loadscope pins an entire MODULE's tests to ONE worker — and scitex-dict's heavy
...
exec nice -n 19 ionice -c 3 \
    ...
    --cov=src/scitex_dict --cov-report=xml --cov-report=term \
    -p no:cacheprovider
